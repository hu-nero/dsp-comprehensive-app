...
// 接收发送
uint16_t CanRecvDrive(Can_TypeDef *Can_Struct, uint16_t Mb, S_CANMSG *s_cRx);		// CAN 接收
uint16_t CanSendDrive(Can_TypeDef *Can_Struct, uint16_t Mb, S_CANMSG *s_cTx);		// CAN发送

extern interrupt void CANA_TxRx_ISR(void);
extern interrupt void CANB_TxRx_ISR(void);
extern interrupt void CANA_Error_ISR(void);
extern interrupt void CANB_Error_ISR(void);
