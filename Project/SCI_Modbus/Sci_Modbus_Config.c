...
#include "Sci_Modbus_config.h"
#include "GPIO\Custom_IOMacro.H"
/*************************************************************/

void Sci_Modbus_init(volatile struct SCI_REGS *p, float LSPCLK_FREQ, float SCI_FREQ)
{
	long SCI_PRD;
...
/**************************************************************/
void Sci_Modbus_RxData(volatile struct SCI_REGS *p, Uint16 *pRxBuffer,Uint16 *pRear,Uint16 RevSize)
{
	Uint16  i;
...
/***********************************************************/
void Sci_Modbus_SendData(volatile struct SCI_REGS *p, Uint16 dat)
{
	p->SCITXBUF = dat;
	while (p->SCICTL2.bit.TXEMPTY == 0);
}

void Sci_Modbus_SendArray(volatile struct SCI_REGS *p, Uint16 * p_Arr, Uint16 Len)
{
	Uint16 i;
	IO_EN1RS485_SEND;
	for(i=0; i<Len; i++)
	{
		Sci_Modbus_SendData(p, *(p_Arr + i) );
	}
	IO_EN1RS485_RECV;
}
